...
func (c *Config) AddTemplate() (err error) {
	...
	color.Cyan(`Select a language (e.g. "4003"): `)
	lang := ""
	for {
  ...
  template, at will replace all placeholders by following rules:
  ...
  $%s%$   Second (e.g. 00)`
	...
	note = `Template will run 3 scripts in sequence when you run "at test":
  ...
  at will replace all placeholders by following rules:

  $%path%$   Path to source file (Excluding $%full%$, e.g. "/home/preet/")
...
func (c *Config) RemoveTemplate() (err error) {
	...
	idx := util.ChooseIndex(0, len(c.Template))
...
func (c *Config) SetDefaultTemplate() error {
	...
	c.Default = util.ChooseIndex(0, len(c.Template))
	return c.save()
}
